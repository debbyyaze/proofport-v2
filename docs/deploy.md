...
Set `NEXT_PUBLIC_APP_URL` to the canonical production HTTPS origin, not a preview deployment URL, so canonical tags, manifest and social preview metadata, `robots.txt`, and `sitemap.xml` all point at the released site.
